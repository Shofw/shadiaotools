...
if  command -v bash > /dev/null; then
	...
	sleep 5
	mount -o rw,remount /system #挂载系统读写                         ln -s /data/data/com.termux/files/usr/bin/bash  /system/bin #>创建链接
    umount /system || mount -o ro,remount /system 2>1 >/dev/null#>卸载或者挂载只>读
    echo
